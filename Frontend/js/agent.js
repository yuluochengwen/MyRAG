...
class AgentChat {
    ...
    setupEventListeners() {
        ...
        if (newSessionMainBtn) {
            newSessionMainBtn.addEventListener("click", () => this.createSession());
        }
        ...
        try {
            ...
            optionsContainer.innerHTML = kbs
                .map((kb) => {
                    ...
                    return `<label class="flex items-center px-2 py-1 rounded hover:bg-gray-50 text-sm text-gray-700"><input type="checkbox" class="mr-2" data-kb-id="${id}"${selected}>${this.escapeHtml(name)} (ID: ${id})</label>`;
                })
                .join("");
    ...
    saveRuntimeSettings() {
        ...
        try {
            localStorage.setItem(this.settingsKey, JSON.stringify(options));
        } catch (error) {
    ...
    renderSessionList() {
        ...
        container.innerHTML = this.sessions
            .map((session) => {
                ...
                return `
                    <button class="agent-session-item w-full text-left p-2 rounded border border-gray-200 hover:border-primary transition ${activeClass}" data-session-id="${session.id}">
        ...
        container.querySelectorAll(".agent-session-item").forEach((btn) => {
    ...
    createLiveStepsPanel() {
        ...
        wrapper.className = "mb-4 text-left";

        const card = document.createElement("div");
        card.className = "inline-block max-w-[80%] bg-gray-50 border border-gray-200 rounded-lg p-4";
        card.innerHTML = `
    ...
    addMessage(role, content, success = true, scroll = true) {
        ...
        bubble.className = `inline-block max-w-[80%] p-4 rounded-lg ${
            role === "user" ? "bg-primary text-white" : success ? "bg-white border border-gray-200" : "bg-red-50 border border-red-200"
        }`;
