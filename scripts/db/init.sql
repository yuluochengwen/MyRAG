...
CREATE TABLE IF NOT EXISTS assistants (
    ...
    lora_model_id INT NULL COMMENT 'LoRA 模型 ID',
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    INDEX idx_lora_model (lora_model_id),
    INDEX idx_status (status),
    INDEX idx_created_at (created_at)
...
-- assistants 表添加 LoRA 外键（幂等处理由 init_db.py 负责）
ALTER TABLE assistants
ADD CONSTRAINT fk_assistants_lora FOREIGN KEY (lora_model_id) REFERENCES lora_models(id) ON DELETE SET NULL;
