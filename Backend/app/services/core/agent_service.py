...
class AgentService:
    ...
    def _register_default_tools(self):
        ...
        if self.kb_service:
            async def search_knowledge_base(query: str, kb_id: int = None, kb_ids: Optional[List[int]] = None, top_k: int = 3) -> str:
                """搜索知识库获取相关信息"""
                try:
                    default_limit = int(getattr(settings, 'agent_default_kb_limit', 50) or 50)
                    resolved_kb_ids = await self._resolve_kb_ids(kb_id=kb_id, kb_ids=kb_ids, limit=default_limit)
        ...
        # 工具4: 日历上下文（用于薪资/考勤天数推导）
        def get_calendar_context() -> str:
            """返回当前与上月的日历信息（含上月工作日天数）。"""
            now = datetime.now()
            year = now.year
            month = now.month
            if month == 1:
            ...
            weekdays_in_last_month = 0
            for day in range(1, days_in_last_month + 1):
                weekday = datetime(last_year, last_month, day).weekday()
                if weekday < 5:
                    weekdays_in_last_month += 1
            ...
            payload = {
                ...
                "weekdays_in_last_month": weekdays_in_last_month,
                "note": "weekdays_in_last_month 仅按周一到周五估算，不含公司排班和法定调休。"
            }
            return json.dumps(payload, ensure_ascii=False)

        self.register_tool(
            name="get_calendar_context",
            description="获取当前与上月日历上下文。用于估算上月应出勤天数和工资计算前置参数。无需参数",
            func=get_calendar_context
        )
    ...
    async def _agent_retrieve_evidence(self, kb_ids: List[int], query: str, top_k: int, use_hybrid: bool) -> List[Dict[str, Any]]:
        ...
        if use_hybrid and bool(getattr(settings.knowledge_graph, 'enabled', False)):
            try:
                from app.services.infrastructure.retrieval.hybrid_retrieval_service import get_hybrid_retrieval_service
        ...
        if hasattr(self.kb_service, "search_knowledge_bases"):
            try:
                ...
                # 多知识库 embedding 配置不一致时，退化为逐库检索再合并，避免直接失败。
                logger.warning("Agent 多库联合检索失败，降级逐库检索: %s", str(error))
...
6. 若问题涉及“上月/本月出勤天数、工作日、按日薪结算”，优先调用 get_calendar_context 获取日期上下文
...
"""
    ...
    def _should_force_kb_lookup(self, user_query: str) -> bool:
        ...
        # 纯时间查询由时间工具优先处理
        if self._should_force_time_tool(query):
            return False
    ...
    def _should_preload_calendar_context(self, user_query: str) -> bool:
        """判断是否应在推理前补充一次日历上下文。"""
        query = (user_query or "").strip().lower()
        if not query:
            return False

        month_markers = ["上月", "上个月", "本月", "上个月的", "last month", "this month"]
        attendance_markers = ["出勤", "工作日", "天数", "工资", "薪资", "日薪", "薪酬", "考勤"]
        return any(m in query for m in month_markers) and any(k in query for k in attendance_markers)
    ...
    def _should_force_calculator_tool(self, user_query: str) -> bool:
        ...
        # 仅出现“乘/除”等词时，要求至少包含两个数值，避免“按日薪乘以出勤天数”这类业务问题被误判为纯计算。
        if has_digit and has_math_kw:
    ...
    async def run(
        ...
        if "get_calendar_context" in self.tools and self._should_preload_calendar_context(user_query):
            calendar_observation = await self.tools["get_calendar_context"].arun()
            tools_called += 1
            if show_steps:
                await record_step({"type": "thought", "content": "该问题涉及月份与出勤天数，先补充日历上下文。"})
                await record_step({"type": "action", "tool": "get_calendar_context", "input": "{}"})
                await record_step({"type": "observation", "content": calendar_observation})
            prompt = self._build_prompt(user_query, history)
            prompt += (
                "\n已补充日历上下文如下，请结合问题进行推理：\n"
                f"{calendar_observation}\n"
                "如信息仍不足，请明确指出缺失参数，不要编造。\n"
            )
        else:
            prompt = self._build_prompt(user_query, history)
